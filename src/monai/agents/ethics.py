    ...
    if llm is not None:
        try:
            ...
            review_prompt = (
                ...
                f"- Scheduling social media posts via official APIs (Twitter/X, Buffer, "
                f"Hootsuite, Later, LinkedIn, Facebook Graph API, etc.) — this is standard "
                f"marketing automation, NOT spam or policy violation\n"
                f"- Writing files for legitimate application code\n"
                f"- Implementing user analytics, A/B testing, and conversion tracking\n"
                f"- SEO keyword research, search volume lookups, and SERP analysis using "
                f"public APIs (Serpstack, SEMrush API, Google Ads API, etc.) — this is "
                f"standard digital marketing, NOT scraping PII\n"
                f"- Fetching publicly available data from APIs (pricing data, product info, "
                f"keyword statistics) — this is market research, NOT mass scraping\n\n"
