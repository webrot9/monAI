...
- **Fix**: (1) Changed `init_strategies()` to insert strategies as `'pending'` instead of `'active'`. (2) Added `validate_strategies()` to `StrategyLifecycle` — checks each strategy's asset requirements (email, domain, payment, platform accounts) against actual DB inventory, activates those that qualify, pauses those that don't. (3) Added `STRATEGY_REQUIREMENTS` dict mapping each strategy to its minimum asset needs. (4) Changed `register_strategy()` to only call `register_brand()` if strategy is actually 'active'. (5) Added `_cleanup_phantom_brands()` to remove brand_social_accounts for non-active strategies. (6) Added `demote_active_without_agent()` to pause DB strategies that have no Python agent registered. (7) Validation runs at startup (after all agents registered) AND at the start of each cycle.
- **Rule**: **NEVER trust DB state from previous sessions.** Strategy activation must be gated on actual asset inventory, not just DB inserts. Every strategy needs a defined set of minimum requirements. Cleanup must run on startup to reconcile DB with reality. When the user reports a bug multiple times, it is the HIGHEST PRIORITY — drop everything else.

### 2026-03-17 - Deep daemon analysis: proxy refresh loop, burn rate disconnected, URL revisits, doomed provisioning
- **Mistake**: (1) Free proxy pool refresh triggered 3x in one cycle (~90s wasted). Each time: scrape 400 candidates, validate 50, find 0 working → pool empty → next request triggers refresh again. (2) Burn rate always €0.0000/day despite active OpenAI spending — `_get_daily_burn_rate()` reads from `cost_log` table, but LLM costs are written to `transactions` table. Different tables! (3) Executor re-browses exact same URLs — steps 5/9, 6/10, 7/8, 1/12 all duplicate URLs. (4) Telegram bot provisioning burns 16 executor steps on impossible task — no SMS service configured, no payment method, but provisioner launches executor anyway.
- **Root cause**: (1) `_validate_batch()` raised unhandled `TimeoutError` → caught as generic `Exception` in `_refresh_pool` → `_initialized` and `_last_refresh` never set → next `get_proxy()` triggers another refresh. (2) `Commercialista._get_daily_burn_rate()` queries `cost_log` table (never written to). `LLM._persist_cost()` writes to `transactions` table. Name mismatch since initial implementation. (3) No URL-level dedup in executor — only domain-level blocking after failure, which also didn't exist before this fix. (4) `_provision_telegram_bot()` had no pre-flight check for SMS/payment prerequisites.
- **Fix**: (1) Set `_initialized=True` and `_last_refresh` in the `except` handler too, preventing infinite re-trigger. (2) Changed `_get_daily_burn_rate()` to query `transactions` table `WHERE type='expense'`. (3) Added `_visited_urls` set to executor — auto-rejects browse to exact URLs already visited. (4) Added pre-flight check in `_provision_telegram_bot()`: if no `sms_api_key` and no payment method, return failed immediately without starting executor.
- **Rule**: Failed initialization must still mark the resource as "attempted" to prevent retry loops. DB table names in queries must be verified against the actual write path — a disconnect between writer and reader is silent data loss. Executor guards must work at both domain level (blocked) AND URL level (already visited). Provisioning tasks with hard external prerequisites (API keys, payment methods) must pre-flight check before launching the expensive executor.

### 2026-03-17 - Executor wastes steps retrying blocked domains, hallucinates credentials, marketing copy as search query
- **Mistake**: (1) Executor burned 16 LLM steps on Telegram bot task: retried DuckDuckGo 3x (all 403), revisited textverified.com 2x, re-searched same query 2x. No in-task memory of failed domains. (2) Steps 14-15: LLM sent `Authorization: Bearer YOUR_API_KEY` literally — placeholder credentials in HTTP requests. (3) Content marketer used `campaign.key_message` as search niche — got "Discover how Nexify Digital can elevate your brand with cutting-edge digital marketing strategies. keywords search volume SEO 2025" as a search query. (4) Graceful shutdown (Ctrl+C) blocked for 13s by proxy validation threads that didn't check shutdown flag.
- **Root cause**: (1) Executor had no within-task tracking of failed domains. The LLM gets action history but doesn't always learn from it. (2) No runtime validation of credentials before HTTP calls — only LLM instruction to "not invent credentials". (3) `key_message` is marketing copy generated by LLM, not a topic/niche keyword. `campaign.get("key_message", strategy)` at content_marketer.py:60 was wrong field. (4) `FreeProxyPool._validate_batch()` ran in daemon thread, didn't check `_shutdown_flag`.
- **Fix**: (1) Added `self._failed_domains` set to executor — tracks domains that fail during browse/http_get/http_post. Auto-rejects requests to known-failed domains. Injects blocked domain list into LLM prompt. (2) Added runtime placeholder detection in http_get/http_post handler — checks header values against common placeholder patterns (YOUR_API_KEY, your_api_key, etc). Returns BLOCKED with clear guidance. (3) Changed to `campaign.get("strategy_name") or campaign.get("target_audience") or strategy`. (4) Imported `_shutdown_flag` in free_proxies validation; checks flag before each proxy test and in the as_completed loop.
- **Rule**: The executor LLM will repeat mistakes unless you build runtime guards. Prompt instructions alone ("don't retry failed domains") are not reliable — enforce with code. Any external-facing HTTP call must validate credentials aren't placeholders. Search queries must come from structured fields (strategy_name, topic), never from free-text marketing copy. Background threads must check the shutdown flag to allow clean exit.

### 2026-03-17 - Web search fails completely: SearXNG JSON disabled, Mojeek parser wrong classes, DDG 403 over Tor
- **Mistake**: (1) SearXNG instances queried with `?format=json` → 302 redirect to homepage (JSON API disabled on most public instances by default). (2) Mojeek parser used `class="ob"` which no longer matches Mojeek's HTML structure → 200 OK but 0 results parsed. (3) DDG lite/html both return 403 over Tor consistently. (4) Executor LLM instructions still hardcoded DDG as first search choice. (5) Free proxy validation timeout too tight (30s) → "futures unfinished" errors.
- **Root cause**: (1) SearXNG docs say JSON format is disabled by default since "it is often misused by bots" — need HTML parsing or POST method. (2) Mojeek changed their HTML structure, old CSS class no longer exists. (3) DDG actively blocks Tor exit nodes now. (4) Old instructions from before TorSearch was centralized. (5) Free proxy validation tests 50 proxies in parallel with 10s each but only waits 30s total.
- **Fix**: (1) Added `_parse_searxng_html()` parser for SearXNG HTML results (`<article class="result">` pattern). Also added POST JSON as last-resort fallback. (2) Added 3 fallback patterns to Mojeek parser: `class="ob"`, `<li class="result">`, and `<h2><a href=...>`. Multiple snippet class patterns tried. (3) Moved DDG lower in priority, SearXNG HTML instances first. Added 6 SearXNG instances. (4) Updated executor.py and base.py instructions to match new engine priority. (5) Increased validation timeout to 45s, added proper TimeoutError handling, cancel unfinished futures.
- **Rule**: Public SearXNG instances should be queried as HTML, not JSON. Search engine parsers need multiple fallback patterns because sites change HTML structure. Always test parsers against sample HTML. When search engine priority changes in TorSearch, also update the LLM instructions in executor.py and base.py.

### 2026-03-17 - Daemon cycle does nothing useful: plans impossible tasks, browsing dead, ethics blocks legit work
- **Mistake**: (1) Orchestrator `plan()` generates SEO/blog/newsletter tasks despite having NO domain, NO payment methods — just 1 email + 1 LinkedIn. LLM wastes cycles on impossible actions. (2) Marketing team `_plan_campaigns()` has zero asset awareness — suggests content marketing campaigns with no website to publish on. (3) Free proxy fallback never activates: when Tor is blocked, `_available_chain()` calls `get_proxy()` on the free pool, gets `None` (pool still refreshing), concludes FREE is "unconfigured" → NAVIGATE ABORTED. (4) LLM ethics review blocks "fetch SEO keyword data" as "scraping without permission" — SEO research isn't in the safe actions whitelist.
- **Root cause**: (1-2) `plan()` context only includes account count, not asset capabilities. No rule tells the LLM "don't plan X without Y". Marketing team prompt has no asset info at all. (3) `_available_chain()` treats FREE like other tiers — checks if a proxy URL exists right now. But FREE needs background refresh time, so it's always empty when first checked as fallback. (4) Ethics LLM review prompt whitelists DOM interaction, Telegram bots, social APIs — but NOT SEO/keyword/market research via APIs.
- **Fix**: (1) Added `asset_summary` (has_domain, has_payment_method, has_email, platforms) to orchestrator context + explicit rules: "no domain = no SEO". (2) Marketing team now queries IdentityManager for accounts and injects asset constraints into prompt. (3) FREE is always included in `_available_chain()` + `get_proxy(wait=True)` blocks up to 30s for pool refresh. (4) Added SEO keyword research, public API data fetching to ethics safe actions whitelist.
- **Rule**: Every LLM prompt that generates actions MUST include what assets are actually available. The LLM has no memory between cycles — if you don't tell it "you have no domain", it will plan website tasks. Proxy fallback tiers that require async initialization (free proxy pool) must be available in the chain regardless of current pool state. Ethics safe actions must cover all legitimate business operations the agent performs.

### 2026-03-17 - Ethics guardrails block ALL legitimate code: Telegram bots, referral programs, HTTP requests
- **Mistake**: The coder's ethics check blocked every Telegram bot code generation attempt: (1) Pattern `"spam"` caught any spec mentioning referral/invite systems. (2) Python dangerous patterns blocked `requests.`, `httpx.`, `urllib.`, `open(`, `eval(`, `exec(`, `importlib` — making it impossible to generate ANY useful Python code (no HTTP, no file I/O). (3) Spec checks used `script_type="python"` for natural language descriptions, so English text triggered code-level pattern matches. (4) LLM ethics review was overprotective — flagged "user data handling" even for standard onboarding flows.
- **Root cause**: Ethics guardrails designed for untrusted browser scripts were applied identically to trusted internal code generation. Patterns like "spam" (substring match after removing spaces) caught "anti-spam", "spam prevention", "reward system" discussions. Python blocks were copied from a sandbox security context where HTTP/file access is dangerous — but the agent NEEDS these to build products.
- **Fix**: (1) Replaced "spam" with specific patterns: "send.spam", "spam.bot", "spam.campaign". (2) Removed Python blocks on standard libraries (requests, httpx, open, etc.) — kept truly dangerous ones (os.system, subprocess, ctypes). (3) Spec check now uses `script_type="spec"` so natural language doesn't trigger code patterns. (4) LLM review prompt explicitly whitelists Telegram bot features, referral systems, A/B testing, HTTP API calls.
- **Rule**: Ethics guardrails must distinguish between UNTRUSTED browser scripts (high risk) and TRUSTED internal code generation (low risk). Pattern matching on single common words ("spam", "track") causes massive false positives — use multi-word patterns or require context. Never block standard library usage (HTTP clients, file I/O) in code the agent generates for its own products.

### 2026-03-17 - LinkedIn phantom account listed in inventory misleads LLM
- **Mistake**: Inventory summary showed "Platform accounts: LinkedIn" even though the account had only a password (no OAuth `access_token` + `person_urn`). The LLM saw this as a usable resource and attempted LinkedIn operations that always failed.
- **Root cause**: `AssetManager.get_inventory()` only checked `status = 'active'` — didn't validate credential completeness. `ensure_platform_account()` validates credentials but only when USE is attempted, not during inventory building. Platform accounts with empty/password-only creds stayed "active" forever.
- **Fix**: (1) `verify_stored_assets()` now checks platform accounts against `get_required_credential_fields()` — marks incomplete accounts with `credential_status=incomplete` in metadata. (2) `summary()` filters out accounts with `credential_status=incomplete` and lists them separately as "Phantom accounts (unusable)". (3) The LLM now sees accurate inventory — no more phantom resources.
- **Rule**: Asset verification must check ALL asset types, not just emails. Platform accounts need credential validation against what the platform API actually requires. The inventory summary is the LLM's reality — if it's wrong, the LLM wastes cycles on impossible tasks.

### 2026-03-17 - System wastes cycles on impossible tasks: Stripe via Tor, orphaned temp emails, phantom assets
- **Mistake**: (1) `_ensure_strategy_payment_providers` attempted Stripe registration through Tor — Stripe blocks Tor, always fails, burns LLM calls + temp emails. (2) `_get_brand_email` created a NEW mail.tm email every cycle because the reuse query used `type = 'email'` but `store_account()` stores as `type = 'platform_account'` — query never matched. Created 68+ orphaned emails. (3) Constraint planner mapped `telegram_bot` goal to only `["email_creation"]` — when email existed, returned 0 steps, never actually created the bot. (4) `verify_stored_assets` counted all emails without `inbox_id` (mail.tm temps) as "unverifiable errors" instead of recognizing them as temp emails or cleaning them up.
- **Root cause**: (1) No Tor-awareness in payment provider auto-provisioning — it blindly dispatched to Stripe/Gumroad/LemonSqueezy regardless of proxy config. (2) DB query `type = 'email'` never matches because `store_account()` hardcodes `type = 'platform_account'` — the correct column to filter is `platform = 'email'`. (3) `_GOAL_TO_ACTIONS` for telegram_bot only had prerequisites (email), not the actual goal action (create bot via BotFather). (4) Asset verification treated all emails the same — no distinction between Mailslurp (verifiable) and mail.tm (disposable, no inbox_id).
- **Fix**: (1) Added `TOR_BLOCKED_PROVIDERS` set and proxy check in `_ensure_strategy_payment_providers` + `_run_api_provisioning`. (2) Fixed all 3 queries using `type = 'email'` to use `platform = 'email'` instead. (3) Added `telegram_bot_creation` standard rule + `_provision_telegram_bot()` handler in provisioner. (4) `verify_stored_assets` now recognizes temp emails (marks verified), suspends orphaned emails without inbox_id (cleanup).
- **Rule**: When writing DB queries, ALWAYS verify the column value matches what the INSERT statement actually uses — don't assume type names, check `store_account()`. Before dispatching any network operation, check if the current proxy config is compatible with the target. Every goal in `_GOAL_TO_ACTIONS` must include the ACTUAL action, not just prerequisites. Asset types must be distinguished in verification — disposable vs persistent resources need different validation.

### 2026-03-16 - Revenue pipeline completely broken: 5 strategies produce €0
- **Mistake**: All 5 active strategies (affiliate, content_sites, lead_gen, newsletter, telegram_bots) created content and Ko-fi checkout links but NEVER recorded revenue. No `payment_ref → strategy` mapping existed — when webhooks fired, the system couldn't connect payments back to strategies. Affiliate and content_sites had no monetization step at all — they published content then looped back to research.
- **Root cause**: (1) `create_checkout_link()` returned payment_ref but nobody stored which strategy created it. (2) Affiliate and content_sites plan() state machines went "published → write_comparison/find_programs" — skipping monetization entirely. (3) Lead_gen, newsletter, telegram_bots created checkouts but never polled for completed payments. (4) Webhook handler couldn't route payments to strategies without the mapping.
- **Fix**: (1) Added `checkout_links` table mapping payment_ref → strategy_name. (2) `create_checkout_link()` auto-stores mapping. (3) Added `monetize_content`/`monetize_article` steps to affiliate/content_sites plan(). (4) Added `check_sales` step to all 5 strategies (polls payment providers). (5) Added `make_checkout_revenue_handler()` for webhook-based revenue recording. (6) `check_pending_sales()` on BaseAgent verifies payments and calls `record_revenue()`.
- **Rule**: Every strategy that creates a checkout link MUST store the payment_ref mapping AND have a subsequent step that checks for completed payments. Revenue recording is not optional — it's the whole point.

### 2026-03-16 - Strategies waste 20-40 LLM calls per cycle on failing browser sessions
- **Mistake**: Each research step (affiliate._research_programs, content_sites._research_keywords, etc.) spawned 3-4 separate browser sessions (ShareASale, CJ, Google Trends, Ubersuggest, Amazon). Via Tor, most failed: Google blocked, ShareASale bot detection, CJ no visible data, Ubersuggest needs JS. Each failed session burned 5-10 LLM calls before the circuit breaker kicked in. Total: 20-40 wasted calls per strategy per cycle.
- **Root cause**: (1) Strategies assumed direct internet access — designed for clearnet, not Tor. (2) No awareness of which sites work via Tor. (3) Each browse_and_extract() spawns a full executor session even when navigation fails on step 1. (4) When all research returned empty, think_json hallucinated data anyway.
- **Fix**: (1) Reduced research steps from 3-4 browser sessions to 1 focused search (DuckDuckGo/Bing work via Tor). (2) Added "no real data" detection — if search returns empty, save "researched" marker and retry next cycle instead of hallucinating. (3) Eliminated browse_and_extract calls to known-failing sites (ShareASale, CJ, Google Trends, Ubersuggest, Amazon).
- **Rule**: Each research step should make AT MOST 1 browser session call. Check if real data was returned before calling think_json. Never hallucinate data when research fails — save a marker and retry next cycle.

### 2026-03-16 - Phantom credentials: system believed it had email/LinkedIn accounts it didn't
- **Mistake**: `register_on_platform()` stored credentials as `{"password": "xyz"}` whenever the browser executor reported "completed" — even when the platform requires OAuth tokens (LinkedIn: `access_token` + `person_urn`, Twitter: `bearer_token`). Since `ensure_platform_account()` only checked `if creds` (non-empty), a password-only dict passed validation. The system then tried to use these phantom accounts, failing every time but never learning.
- **Root cause**: (1) No platform-specific credential requirements — any non-empty dict was accepted. (2) No credential validation against real APIs. (3) LLM executor could hallucinate "completed" status for form fills that didn't actually succeed. (4) No self-healing — auth failures at publish time didn't deactivate the account, so the system kept retrying with bad creds.
- **Fix**: (1) Added `required_credential_fields` to each platform client (e.g. LinkedIn requires `access_token` + `person_urn`). (2) `ensure_platform_account()` now checks platform-specific required fields, not just non-empty. (3) `setup_account()` rejects accounts with missing required fields instead of marking them active. (4) `register_on_platform()` refuses to even attempt browser registration for platforms that need OAuth (can't get API tokens from form fill). (5) Self-healing: auth failures at publish time mark the account as `auth_failed`; auth failures in `platform_action()` mark identity as `stale`. (6) Added `validate_credentials()` to each platform client for real API-level verification.
- **Rule**: Never trust browser automation "completed" as proof of valid credentials. Define what each platform API actually requires and enforce it at storage time. When credentials fail at runtime, deactivate immediately — don't keep retrying. Platforms requiring OAuth tokens cannot be auto-provisioned via form fill.
